...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t vecDescriptor;
  cusparseDnVecDescr_t vecDescriptorOut;
  ...
  cusparseCheck(cusparseCreateDnVec(&vecDescriptor, dimX, temp, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&vecDescriptorOut, dimX, tmp, CUDA_R_64F));
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, vecDescriptor, &zero, vecDescriptorOut, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, &bufferSize));
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    //@@ tmp = 1 * A * temp + 0 * tmp
    cusparseCheck(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, Adescriptor, vecDescriptor, &zero, vecDescriptorOut, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, buffer));
  ...
  cputimer_stop("Calculation time: ");
  ...
  cusparseCheck(cusparseDestroyDnVec(vecDescriptor));
  cusparseCheck(cusparseDestroyDnVec(vecDescriptorOut));
